...
date: 2024-07-07
published: true
blogImage: "/BlogAssets/PrettierTypescriptNx.png"

---

Welcome to this comprehensive guide on setting up Prettier in a monorepo! In this blog, I'll walk you through the steps to manage formatting, linting, and type-checking across all projects and packages in your monorepo. We'll leverage several tools to streamline our development workflow:
...
## This is the final folder structure which i will be referencing 
```js
├── [4.0K]  apps/
│   ├── [4.0K]  api/
│   │   ├── [4.0K]  dist/  
...
- **barebone TypeScript**: A sample library to test a local library. you can checkout my blog on setting up typescript from scratch for a mono repo [Blog](https://artemisio.vercel.app/blog/Setting-up-typescript-for-a-monorepo-from-scratch)
...
The `nx` command will prompt you to choose the order in which you want to run the scripts. Arrange them according to your preference. Keep in mind this creates an `nx.json` file, allowing easy addition or editing of scripts in the future. Ensure caching is enabled. Here is my `nx.json`:
...
![](/BlogAssets/Pasted-image-20240705144117.png)
...
2. **tsc**: The TypeScript compiler which outputs the compiled TypeScript code in CommonJS format. [Check out the TODO section for a detailed TypeScript setup](#).
